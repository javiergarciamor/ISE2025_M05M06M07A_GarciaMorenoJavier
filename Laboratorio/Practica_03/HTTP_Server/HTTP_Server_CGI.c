...
extern bool LEDrun;
extern void LCD_write(uint8_t line, char str[]);
      ...
      switch (env[2]) {
        ...
        case '3':
          ...
          len = (uint32_t)sprintf (buf, &env[4], adv);
				case '4':
          adv2 = AD_in (1);
          len = (uint32_t)sprintf (buf, &env[4], adv2);
          break;
        case '5':
          len = (uint32_t)sprintf (buf, &env[4], (double)((float)adv2*3.3f)/4096);
          break;
        case '6':
          adv2 = (adv2 * 100) / 4096;
          len = (uint32_t)sprintf (buf, &env[4], adv2);
